...
int main() {
    ...
    scene->_objects.push_back(std::make_shared<RayTracer::Sphere>(Math::Vector3D(-3, 4, 0), 4, basicPlastic));
    ...
    scene->_lights.push_back(std::make_shared<RayTracer::Spot>(Math::Vector3D(0, 40, -10), Math::Vector3D(70, 70, 70)));

    scene->_camera = std::make_unique<RayTracer::Camera>(Math::Vector3D(-3, 3, -25), 800, 800, 110);
